...
#include <cstring>  ///< Necessário para manipulação de strings C
#include <arpa/inet.h>  // Para socket UDP
...
int main() {
    try {
        ...
        serverAddr.sin_port = htons(45200);
        if (inet_pton(AF_INET, "192.168.42.10", &serverAddr.sin_addr) <= 0) {
        ...
        while (true) {
            ...
            // Monta mensagem em texto
            std::string message = "Corrente: " + std::to_string(current) + " A";
           ...
           //******************************************************/
            ...
            if (sent < 0) {
                ...
                std::cout << "Enviado para 192.168.42.10:45200 -> " << message << std::endl;
            }
